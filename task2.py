...
def evaluate_model(model, tokenizer, test_loader, device, pathologies=None):
    ...
    if MATPLOTLIB_AVAILABLE:
        try:
            ...
            plt.figure(figsize=(12, 10))
            thresholds_by_class = [m['threshold'] for m in class_metrics]
            pos_ratios = []
            
            # Calculate positive ratios for each class
            for i in range(all_labels.shape[1]):
                mask = valid_indices[:, i]
                y_true = all_labels[mask, i]
                pos_ratio = np.sum(y_true) / len(y_true) if len(y_true) > 0 else 0
            ...
            # Create scatter plot of threshold vs positive ratio
            plt.scatter(pos_ratios, thresholds_by_class, s=100, alpha=0.7)
            
            # Add labels for each point
            for i, (x, y) in enumerate(zip(pos_ratios, thresholds_by_class)):
                plt.annotate(pathologies[i], (x, y), 
                             xytext=(5, 5), textcoords='offset points')
            
            plt.xlabel('Positive Ratio')
            plt.ylabel('Optimal Threshold')
            plt.title('Class Imbalance vs Optimal Threshold')
            plt.grid(True)
            plt.savefig('threshold_vs_imbalance.png')
            print("Class Imbalance vs Optimal Threshold chart saved to 'threshold_vs_imbalance.png'")
